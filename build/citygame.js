...
(function (global, factory) {
	...
	const Micro = {
		...
		BudgetProps: ['autoBudget', 'totalFunds', 'policePercent', 'roadPercent', 'firePercent', 'roadSpend', 'policeSpend', 'fireSpend', 'roadMaintenanceBudget', 'policeMaintenanceBudget', 'fireMaintenanceBudget', 'cityTax', 'roadEffect', 'policeEffect', 'fireEffect'],
		// eval
		...
		MAP_WIDTH: 128,
		MAP_HEIGHT: 128,
		MAP_DEFAULT_WIDTH: 128 * 3,
		//Micro.MAP_WIDTH*3,
		MAP_DEFAULT_HEIGHT: 128 * 3,
		//Micro.MAP_HEIGHT*3,
		MAP_BIG_DEFAULT_WIDTH: 128 * 16,
		//Micro.MAP_WIDTH*16,
		MAP_BIG_DEFAULT_HEIGHT: 128 * 16,
		//Micro.MAP_HEIGHT*16,
	...
	class math {
		...
		static getRandom16Signed() {
			...
			if (value < 32768) return value;else return -(2 ** 16) + value;
		}
	}
	...
	class ZoneUtils {
		...
		static canBulldoze(tile) {
			...
			return tile >= Tile.FIRSTRIVEDGE && tile <= Tile.LASTRUBBLE || tile >= Tile.POWERBASE + 2 && tile <= Tile.POWERBASE + 12 || tile >= Tile.TINYEXP && tile <= Tile.LASTTINYEXP + 2;
		}
		...
		static checkZoneSize(tile) {
			if (tile >= Tile.RESBASE - 1 && tile <= Tile.PORTBASE - 1 || tile >= Tile.LASTPOWERPLANT + 1 && tile <= Tile.POLICESTATION + 4 || tile >= Tile.CHURCH1BASE && tile <= Tile.CHURCH7LAST) {
				return 3;
			}
			if (tile >= Tile.PORTBASE && tile <= Tile.LASTPORT || tile >= Tile.COALBASE && tile <= Tile.LASTPOWERPLANT || tile >= Tile.STADIUMBASE && tile <= Tile.LASTZONE) {
				return 4;
			}
			return 0;
		}
		static fireZone(map, x, y, blockMaps) {
			...
			if (tileValue === Tile.AIRPORT) zoneSize = 5;else if (tileValue >= Tile.PORTBASE) zoneSize = 3;else if (tileValue < Tile.PORTBASE) zoneSize = 2;
	...
	ZoneUtils.checkBigZone = function (tile) {
		let result;
		switch (tile) {
	...
	const Tile = {
		...
		BLBNBIT: 0x1000 | 0x2000,
		//BULLBIT | BURNBIT,
		BLBNCNBIT: 0x1000 | 0x2000 | 0x4000,
		//BULLBIT | BURNBIT | CONDBIT,
		BNCNBIT: 0x2000 | 0x4000,
		//BURNBIT | CONDBIT,
		ASCBIT: 0x0800 | 0x4000 | 0x2000,
		// ANIMBIT | CONDBIT | BURNBIT,
		ALLBITS: 0x8000 | 0x4000 | 0x2000 | 0x1000 | 0x0800 | 0x0400,
		...
		LASTRIVEDGE: 20,
		WATER_LOW: 2,
		//RIVER,	 // First water tile
		WATER_HIGH: 20,
		//LASTRIVEDGE, // Last water tile (inclusive)
		...
		UNUSED_TRASH2: 39,
		WOODS_HIGH: 39,
		//UNUSED_TRASH2, // Why is an 'UNUSED' tile used?
		WOODS2: 40,
		WOODS3: 41,
		WOODS4: 42,
		WOODS5: 43,
		...
		RESBASE: 240,
		// Empty residential, tiles 240--248
		FREEZ: 244,
		// center-tile of 3x3 empty residential
		...
		HOSPITALBASE: 405,
		// Center of hospital (tiles 405--413)
		HOSPITAL: 409,
		// Center of hospital (tiles 405--413)

		CHURCHBASE: 414,
		// Center of church (tiles 414--422)
		CHURCH0BASE: 414,
		// numbered alias
		CHURCH: 418,
		// Center of church (tiles 414--422)
	...
	const RoadTable = [Tile.ROADS, Tile.ROADS2, Tile.ROADS, Tile.ROADS3, Tile.ROADS2, Tile.ROADS2, Tile.ROADS4, Tile.ROADS8, Tile.ROADS, Tile.ROADS6, Tile.ROADS, Tile.ROADS7, Tile.ROADS5, Tile.ROADS10, Tile.ROADS9, Tile.INTERSECTION];
	const RailTable = [Tile.LHRAIL, Tile.LVRAIL, Tile.LHRAIL, Tile.LVRAIL2, Tile.LVRAIL, Tile.LVRAIL, Tile.LVRAIL3, Tile.LVRAIL7, Tile.LHRAIL, Tile.LVRAIL5, Tile.LHRAIL, Tile.LVRAIL6, Tile.LVRAIL4, Tile.LVRAIL9, Tile.LVRAIL8, Tile.LVRAIL10];
	const WireTable = [Tile.LHPOWER, Tile.LVPOWER, Tile.LHPOWER, Tile.LVPOWER2, Tile.LVPOWER, Tile.LVPOWER, Tile.LVPOWER3, Tile.LVPOWER7, Tile.LHPOWER, Tile.LVPOWER5, Tile.LHPOWER, Tile.LVPOWER6, Tile.LVPOWER4, Tile.LVPOWER9, Tile.LVPOWER8, Tile.LVPOWER10];
	...
	var disasterMessages = [Messages.EARTHQUAKE, Messages.EXPLOSION_REPORTED, Messages.FIRE_REPORTED, Messages.FLOODING_REPORTED, Messages.MONSTER_SIGHTED, Messages.NUCLEAR_MELTDOWN, Messages.TORNADO_SIGHTED];
	...
	const Text = function () {
		...
		var zoneTypes = ['Clear', 'Water', 'Trees', 'Rubble', 'Flood', 'Radioactive Waste', 'Fire', 'Road', 'Power', 'Rail', 'Residential', 'Commercial', 'Industrial', 'Seaport', 'Airport', 'Coal Power', 'Fire Department', 'Police Department', 'Stadium', 'Nuclear Power', 'Draw Bridge', 'Radar Dish', 'Fountain', 'Industrial', 'Steelers 38	Bears 3', 'Draw Bridge', 'Ur 238'];
	...
	class SpriteUtils {
		...
		static turnTo(presentDir, desiredDir) {
			...
			if (presentDir < desiredDir) {
				...
				if (desiredDir - presentDir < 4) presentDir++;else presentDir--;
			} else {
				if (presentDir - desiredDir < 4) presentDir--;else presentDir++;
			}
		...
		static getDir(orgX, orgY, destX, destY) {
			...
			if (deltaX * 2 < deltaY) i++;else if (deltaY * 2 < deltaX) i--;
			if (i < 0 || i > 12) i = 0;
		...
		static checkWet(tileValue) {
			if (tileValue === Tile.HPOWER || tileValue === Tile.VPOWER || tileValue === Tile.HRAIL || tileValue === Tile.VRAIL || tileValue === Tile.BRWH || tileValue === Tile.BRWV) return true;else return false;
		}
		static destroyMapTile(spriteManager, map, blockMaps, ox, oy) {
			...
			if (SpriteUtils.checkWet(tileValue)) map.setTile(x, y, Tile.RIVER, 0);else map.setTile(x, y, Tile.TINYEXP, Tile.BULLBIT | Tile.ANIMBIT);
		}
				...
				for (let i = dir; i < dir + 4; i++) {
					...
					if (tileValue >= Tile.RAILBASE && tileValue <= Tile.LASTRAIL || tileValue === Tile.RAILVPOWERH || tileValue === Tile.RAILHPOWERV) {
						if (this.dir !== dir2 && this.dir !== this.CANTMOVE) {
							if (this.dir + dir2 === this.WEST) this.frame = this.NWSE;else this.frame = this.NESW;
						} else {
	...
	class BoatSprite extends BaseSprite {
		constructor(map, spriteManager, x, y) {
			...
			this.xOffset = -24;
			this.yOffset = -24;
			if (x < SpriteUtils.worldToPix(4)) this.frame = 3;else if (x >= SpriteUtils.worldToPix(map.width - 4)) this.frame = 7;else if (y < SpriteUtils.worldToPix(4)) this.frame = 5;else if (y >= SpriteUtils.worldToPix(map.height - 4)) this.frame = 1;else this.frame = 3;
	...
	class MonsterSprite extends BaseSprite {
		constructor(map, spriteManager, x, y) {
			...
			if (x > SpriteUtils.worldToPix(map.width) / 2) {
				if (y > SpriteUtils.worldToPix(map.height) / 2) this.frame = 10;else this.frame = 7;
		...
		move(spriteCycle, messageManager, disasterManager, blockMaps) {
			...
			if (currentDir < 4) {
				...
				if (dir !== currentDir && math.getChance(10)) {
					if (math.getRandom16() & 1) frame = this.cardinals1[currentDir];else frame = this.cardinals2[currentDir];
					currentDir = 4;
					if (!this.soundCount) {
				...
				if (!(math.getRandom16() & 3)) {
					if (math.getRandom16() & 1) frame = this.diagonals1[frame];else frame = this.diagonals2[frame];
			...
			for (let i = 0; i < spriteList.length; i++) {
				let s = spriteList[i];
				if (s.frame !== 0 && (s.type === Micro.SPRITE_AIRPLANE || s.type === Micro.SPRITE_HELICOPTER || s.type === Micro.SPRITE_SHIP || s.type === Micro.SPRITE_TRAIN) && SpriteUtils.checkSpriteCollision(this, s)) s.explodeSprite(messageManager);
			}
			SpriteUtils.destroyMapTile(this.spriteManager, this.map, blockMaps, this.x, this.y);
		}
	}
			...
			if (this.soundCount === 0) {
				...
				if (x >= 0 && x < this.map.width && y >= 0 && y < this.map.height) {
					if (blockMaps.trafficDensityMap.worldGet(x, y) > 170 && (math.getRandom16() & 7) === 0) {
						messageManager.sendMessage(Messages.HEAVY_TRAFFIC, {
							x: x,
							y: y
						});
	...
	class TornadoSprite extends BaseSprite {
		...
		move(spriteCycle, messageManager, disasterManager, blockMaps) {
			...
			// value
			if (frame === 2) {
				if (this.flag) frame = 3;else frame = 1;
			} else {
				if (frame === 1) this.flag = 1;else this.flag = 0;
				frame = 2;
			}
			if (this.count > 0) this.count--;
			this.frame = frame;
			...
			for (let i = 0; i < spriteList.length; i++) {
				...
				if (s.frame !== 0 && (s.type === Micro.SPRITE_AIRPLANE || s.type === Micro.SPRITE_HELICOPTER || s.type === Micro.SPRITE_SHIP || s.type === Micro.SPRITE_TRAIN) && SpriteUtils.checkSpriteCollision(this, s)) {
					s.explodeSprite(messageManager);
				}
			}
	...
	*/
		...
		move(spriteCycle, messageManager, disasterManager, blockMaps) {
			if ((spriteCycle & 1) === 0) {
				if (this.frame === 1) {
					...
					messageManager.sendMessage(Messages.EXPLOSION_REPORTED, {
						x: explosionX,
						y: explosionY
					});
				}
	...
	class SpriteManager {
		...
		getSpritesInView(startX, startY, lastX, lastY) {
			...
			return this.spriteList.filter(function (s) {
				return s.x + s.xOffset >= startX && s.y + s.yOffset >= startY && !(s.x + s.xOffset >= lastX && s.y + s.yOffset >= lastY);
			});
		}
		moveObjects(simData) {
		...
		makeMonsterAt(messageManager, x, y) {
			...
			messageManager.sendMessage(Messages.MONSTER_SIGHTED, {
				x: x,
				y: y
			});
		}
	...
	class Evaluation {
		...
		save(saveData) {
			for (var i = 0, l = Micro.EvalProps.length; i < l; i++) saveData[Micro.EvalProps[i]] = this[Micro.EvalProps[i]];
		}
		load(saveData) {
			for (var i = 0, l = Micro.EvalProps.length; i < l; i++) this[Micro.EvalProps[i]] = saveData[Micro.EvalProps[i]];
		}
		...
		evalInit() {
			...
			for (i = 0; i < Micro.NUMPROBLEMS; i++) this.problemVotes[i] = {
				index: i,
				voteCount: 0
			};
		...
		getProblemNumber(i) {
			if (i < 0 || i >= Micro.NUM_COMPLAINTS || this.problemOrder[i] === Micro.NUMPROBLEMS) return -1;else return this.problemOrder[i];
		}
		getProblemVotes(i) {
			if (i < 0 || i >= Micro.NUM_COMPLAINTS || this.problemOrder[i] == Micro.NUMPROBLEMS) return -1;else return this.problemVotes[this.problemOrder[i]].voteCount;
		}
	}
	...
	class Valves {
		...
		setValves(gameLevel, census, budget) {
			...
			if (labourBase > 0.0) labourBase = census.resHist10[1] / labourBase;else labourBase = 1;
			...
			var resRatio;
			if (normalizedResPop > 0) resRatio = projectedResPop / normalizedResPop;else resRatio = resRatioDefault;
			var comRatio;
			if (census.comPop > 0) comRatio = projectedComPop / census.comPop;else comRatio = projectedComPop;
			var indRatio;
			if (census.indPop > 0) indRatio = projectedIndPop / census.indPop;else indRatio = projectedIndPop;
	...
	class Budget {
		...
		load(saveData) {
			for (var i = 0, l = Micro.BudgetProps.length; i < l; i++) this[Micro.BudgetProps[i]] = saveData[Micro.BudgetProps[i]];
	...
	class Census {
		...
		save(saveData) {
			for (var i = 0, l = Micro.CensusProps.length; i < l; i++) saveData[Micro.CensusProps[i]] = this[Micro.CensusProps[i]];
		}
		load(saveData) {
			for (var i = 0, l = Micro.CensusProps.length; i < l; i++) this[Micro.CensusProps[i]] = saveData[Micro.CensusProps[i]];
		}
		clearCensus() {
		...
		take10Census(budget) {
			...
			this.resPop >> 8;
			if (this.hospitalPop < this.resPopScaled) this.needHospital = 1;else if (this.hospitalPop > this.resPopScaled) this.needHospital = -1;else if (this.hospitalPop === this.resPopScaled) this.needHospital = 0;
			this.changed = true;
		}
		take120Census() {
	...
	class Position {
		constructor(pos, deltaX, deltaY) {
			...
			this.x = 0;
			this.y = 0;
			this.validDirs = [Direction.NORTH, Direction.NORTHEAST, Direction.EAST, Direction.SOUTHEAST, Direction.SOUTH, Direction.SOUTHWEST, Direction.WEST, Direction.NORTHWEST, Direction.INVALID];
			...
			if (this.isNumber(pos)) {
				...
				this.set(pos);
				if (arguments.length === 2) moveOK = this.move(deltaX);else if (arguments.length === 3) {
					this.x += deltaX;
					this.y += deltaY;
				}
			}
		...
		move(dir) {
			...
			switch (dir) {
				...
				case Direction.SOUTHEAST:
					if (this.y < this.height - 1 && this.x < this.width - 1) {
						this.x++;
						this.y++;
						up = true;
					}
				...
				case Direction.SOUTHWEST:
					if (this.y < this.height - 1 && this.x > 0) {
						this.y++;
						this.x--;
						up = true;
					}
	...
	class PowerManager {
		...
		nuclearPowerFound(map, x, y, simData) {
			...
			// Ensure animation bits set	 no animation for 3d
			if (!simData.is3D) for (var i = 0; i < 4; i++) map.addTileFlags(x, y, Tile.ANIMBIT | Tile.CONDBIT | Tile.POWERBIT | Tile.BURNBIT);
		}
		...
		repairZone(x, y, zoneSize) {
			...
			for (yy = -1; yy < zoneSize - 1; yy++) {
				for (xx = -1; xx < zoneSize - 1; xx++) {
					...
					if (currentValue < Tile.RUBBLE || currentValue >= Tile.ROADBASE) this._map.setTile(x + xx, y + yy, tileValue, Tile.CONDBIT | Tile.BURNBIT);
				}
			}
		}
		checkTile(x, y, cityTime) {
			let i = this._actions.length,
				current,
				period,
				tile,
				tileValue,
	...
	class DisasterManager {
		...
		makeEarthquake() {
			...
			EventEmitter.emitEvent(Messages.EARTHQUAKE, {
			...
			for (i = 0; i < strength; i++) {
				...
				if (vulnerable(this._map.getTile(x, y))) {
					if ((i & 0x3) !== 0) this._map.setTo(x, y, ZoneUtils.randomRubble());else this._map.setTo(x, y, ZoneUtils.randomFire());
				}
			}
		}
			...
			for (i = 0; i < times; i++) {
				...
				if (!tile.isZone()) {
					...
					if (tile > lowerLimit && tile < Tile.LASTZONE) {
						...
						EventEmitter.emitEvent(Messages.FIRE_REPORTED, {
							showable: true,
							x: x,
							y: y
						});
				...
				if (tileValue > Tile.CHANNEL && tileValue <= Tile.WATER_HIGH) {
					for (j = 0; j < 4; j++) {
						...
						if (tile === Tile.DIRT || tile.isBulldozable() && tile.isCombustible) {
							...
							this._floodCount = 30;
							EventEmitter.emitEvent(Messages.FLOODING_REPORTED, {
								showable: true,
								x: xx,
								y: yy
							});
		...
		doMeltdown(x, y) {
			...
			EventEmitter.emitEvent(Messages.NUCLEAR_MELTDOWN, {
				showable: true,
				x: x,
				y: y
			});
	...
	const Residential = {
		...
		buildHouse: function (map, x, y, lpValue) {
			...
			if (best > 0 && map.testBounds(x + xDelta[best], y + yDelta[best])) map.setTile(x + xDelta[best], y + yDelta[best], Tile.HOUSE + math.getRandom(2) + lpValue * 3, Tile.BLBNCNBIT);
		...
		evalResidential: function (blockMaps, x, y, traffic) {
			...
			let landValue = blockMaps.landValueMap.worldGet(x, y);
			landValue -= blockMaps.pollutionDensityMap.worldGet(x, y);
			if (landValue < 0) landValue = 0;else landValue = Math.min(landValue * 32, 6000);
	...
	const Industrial = {
		...
		degradeZone: function (map, x, y, blockMaps, populationCategory, valueCategory, zonePower) {
			// Note that we special case empty zones here, rather than having to check population value on every
			// call to placeIndustrial (which we anticipate will be called more often)
			if (populationCategory > 1) Industrial.placeIndustrial(map, x, y, populationCategory - 2, valueCategory, zonePower);else ZoneUtils.putZone(map, x, y, Tile.INDCLR, zonePower);
			ZoneUtils.incRateOfGrowth(blockMaps, x, y, -8);
		},
		...
		industrialFound: function (map, x, y, simData) {
			...
			if (math.getChance(7)) {
				...
				if (zoneScore < 350 && zoneScore + 26380 < math.getRandom16Signed()) Industrial.degradeZone(map, x, y, simData.blockMaps, population, math.getRandom16() & 1, zonePower);
			}
		}
	};
	...
	const Road = {
		...
		doBridge: function (map, x, y, currentTile, simData) {
			...
			if (currentTile === Tile.BRWV) {
				...
				if (math.getChance(3) && simData.spriteManager.getBoatDistance(x, y) > 340) Road.closeBridge(map, x, y, verticalDeltaX, verticalDeltaY, openVertical, closeVertical);
				return true;
			}
			if (currentTile == Tile.BRWH) {
				// We have an open horizontal bridge. Possibly close it.
				if (math.getChance(3) && simData.spriteManager.getBoatDistance(x, y) > 340) Road.closeBridge(map, x, y, horizontalDeltaX, horizontalDeltaY, openHorizontal, closeHorizontal);
				return true;
			}
				...
				if (math.getChance(511)) {
					...
					if (!currentTile.isConductive()) {
						if (simData.budget.roadEffect < (math.getRandom16() & 31)) {
							...
							if ((tileValue & 15) < 2 || (tileValue & 15) === 15) map.setTile(x, y, Tile.RIVER);else map.setTo(x, y, ZoneUtils.randomRubble());
							return;
						}
					}
				}
			...
			if (simData.budget.shouldDegradeRoad()) {
				if (math.getChance(511)) {
					...
					if (simData.budget.roadEffect < (math.getRandom16() & 31)) {
						...
						// Replace bridge tiles with water, otherwise rubble
						if (mapValue < Tile.RAILBASE + 2) map.setTile(x, y, Tile.RIVER, 0);else map.setTo(x, y, ZoneUtils.randomRubble());
					}
				}
			}
		},
	...
	*/
	...
	const MiscTiles = {
		...
		fireFound: function (map, x, y, simData) {
			...
			if (i > 100) rate = 1;else if (i > 20) rate = 2;else if (i > 0) rate = 3;
	...
	class MapUtils {
		...
		static neutraliseTrafficMap(blockMaps) {
			...
			while (x--) {
				...
				while (y--) {
					value = bm.get(x, y);
					if (value !== 0) {
						if (value <= 24) value = 0;else if (value > 200) value = value - 34;else value = value - 24;
						bm.set(x, y, value);
					}
				}
			}
		...
		static getCityCentreDistance(map, x, y) {
			let xDis, yDis;
			if (x > map.cityCentreX) xDis = x - map.cityCentreX;else xDis = map.cityCentreX - x;
			if (y > map.cityCentreY) yDis = y - map.cityCentreY;else yDis = map.cityCentreY - y;
	...
	class Simulation {
		...
		save(saveData) {
			for (let i = 0, l = Micro.savePropsVar.length; i < l; i++) saveData[Micro.savePropsVar[i]] = this[Micro.savePropsVar[i]];
		...
		load(saveData) {
			...
			for (let i = 0, l = Micro.savePropsVar.length; i < l; i++) this[Micro.savePropsVar[i]] = saveData[Micro.savePropsVar[i]];
			...
			switch (this.phaseCycle) {
				...
				case 9:
					...
					if (this.cityTime % Micro.TAX_FREQUENCY === 0) {
						this.budget.collectTax(this.gameLevel, this.census);
						this.evaluation.cityEvaluation();
					}
					break;
				case 10:
					if (this.simCycle % 5 === 0) {
						MapUtils.neutraliseRateOfGrowthMap(this.blockMaps);
					}
					MapUtils.neutraliseTrafficMap(this.blockMaps);
					this.sendMessages();
					break;
				case 11:
					if (this.simCycle % Micro.speedPowerScan[speedIndex] === 0) this.powerManager.doPowerScan(this.census);
					break;
				case 12:
					if (this.simCycle % Micro.speedPollutionTerrainLandValueScan[speedIndex] === 0) MapUtils.pollutionTerrainLandValueScan(this.map, this.census, this.blockMaps);
					break;
				case 13:
					if (this.simCycle % Micro.speedCrimeScan[speedIndex] === 0) MapUtils.crimeScan(this.census, this.blockMaps);
					break;
				case 14:
					if (this.simCycle % Micro.speedPopulationDensityScan[speedIndex] === 0) MapUtils.populationDensityScan(this.map, this.blockMaps);
					break;
				case 15:
					if (this.simCycle % Micro.speedFireAnalysis[speedIndex] === 0) MapUtils.fireAnalysis(this.blockMaps);
					this.disasterManager.doDisasters(this.census);
					break;
			}
		...
		sendMessages() {
			...
			switch (this.cityTime & 63) {
				case 1:
					if (Math.floor(totalZonePop / 4) >= this.census.resZonePop) this.messageManager.sendMessage(Messages.NEED_MORE_RESIDENTIAL);
					break;
				case 5:
					if (Math.floor(totalZonePop / 8) >= this.census.comZonePop) this.messageManager.sendMessage(Messages.NEED_MORE_COMMERCIAL);
					break;
				case 10:
					if (Math.floor(totalZonePop / 8) >= this.census.indZonePop) this.messageManager.sendMessage(Messages.NEED_MORE_INDUSTRIAL);
					break;
				case 14:
					if (totalZonePop > 10 && totalZonePop * 2 > this.census.roadTotal) this.messageManager.sendMessage(Messages.NEED_MORE_ROADS);
					break;
				case 18:
					if (totalZonePop > 50 && totalZonePop > this.census.railTotal) this.messageManager.sendMessage(Messages.NEED_MORE_RAILS);
					break;
				case 22:
					if (totalZonePop > 10 && powerPop == 0) this.messageManager.sendMessage(Messages.NEED_ELECTRICITY);
					break;
				case 26:
					if (this.census.resPop > 500 && this.census.stadiumPop === 0) {
						this.messageManager.sendMessage(Messages.NEED_STADIUM);
						this.valves.resCap = true;
					} else {
						this.valves.resCap = false;
					}
					break;
				case 28:
					if (this.census.indPop > 70 && this.census.seaportPop === 0) {
						this.messageManager.sendMessage(Messages.NEED_SEAPORT);
						this.valves.indCap = true;
					} else {
						this.valves.indCap = false;
					}
					break;
				case 30:
					if (this.census.comPop > 100 && this.census.airportPop === 0) {
						this.messageManager.sendMessage(Messages._NEED_AIRPORT);
						this.valves.comCap = true;
					} else {
						this.valves.comCap = false;
					}
					break;
				case 32:
					let zoneCount = this.census.unpoweredZoneCount + this.census.poweredZoneCount;
					if (zoneCount > 0) {
						if (this.census.poweredZoneCount / zoneCount < 0.7) this.messageManager.sendMessage(Messages.BLACKOUTS_REPORTED);
					}
					break;
				case 35:
					if (this.census.pollutionAverage > 60) this.messageManager.sendMessage(Messages.HIGH_POLLUTION);
					break;
				case 42:
					if (this.census.crimeAverage > 100) this.messageManager.sendMessage(Messages.HIGH_CRIME);
					break;
				case 45:
					if (this.census.totalPop > 60 && this.census.fireStationPop === 0) this.messageManager.sendMessage(Messages.NEED_FIRE_STATION);
					break;
				case 48:
					if (this.census.totalPop > 60 && this.census.policeStationPop === 0) this.messageManager.sendMessage(Messages.NEED_POLICE_STATION);
					break;
				case 51:
					if (this.budget.cityTax > 12) this.messageManager.sendMessage(Messages.TAX_TOO_HIGH);
					break;
				case 54:
					if (this.budget.roadEffect < Math.floor(5 * this.budget.MAX_ROAD_EFFECT / 8) && this.census.roadTotal > 30) this.messageManager.sendMessage(Messages.ROAD_NEEDS_FUNDING);
					break;
				case 57:
					if (this.budget.fireEffect < Math.floor(7 * this.budget.MAX_FIRE_STATION_EFFECT / 10) && this.census.totalPop > 20) this.messageManager.sendMessage(Messages.FIRE_STATION_NEEDS_FUNDING);
					break;
				case 60:
					if (this.budget.policeEffect < Math.floor(7 * this.budget.MAX_POLICE_STATION_EFFECT / 10) && this.census.totalPop > 20) this.messageManager.sendMessage(Messages.POLICE_NEEDS_FUNDING);
					break;
				case 63:
					if (this.census.trafficAverage > 60) this.messageManager.sendMessage(Messages.TRAFFIC_JAMS, -1, -1, true);
					break;
			}
		}
		checkGrowth() {
				...
				if (lastClass !== newClass) {
					switch (newClass) {
						...
						case Micro.CC_CITY:
							...
							break;
						case Micro.CC_CAPITAL:
							message = Messages.REACHED_CAPITAL;
							break;
						case Micro.CC_METROPOLIS:
							message = Messages.REACHED_METROPOLIS;
							break;
						case Micro.CC_MEGALOPOLIS:
		...
		updateTime() {
			...
			if (this._cityYearLast !== cityYear || this._cityMonthLast !== cityMonth) {
				...
				this.messageManager.sendMessage(Messages.DATE_UPDATED, {
					month: cityMonth,
					year: cityYear
				});
			}
	...
	class GameMap {
		...
		load(saveData) {
			...
			while (i < lng) {
				...
				if (isOld) this.setTileValue(x, y, map[i].value);else this.setTileValue(x, y, map[i]);
				i++;
			}
		...
		putZone(centreX, centreY, centreTile, size) {
			...
			for (y = startY; y < startY + size; y++) {
				for (x = startX; x < startX + size; x++) {
					if (x === centreX && y === centreY) this.setTo(x, y, new Tiles(tile, Tile.BNCNBIT | Tile.ZONEBIT));else this.setTo(x, y, new Tiles(tile, Tile.BNCNBIT));
					tile += 1;
				}
			}
		}
	...
	class MapGenerator {
		constructor() {
			this.SRMatrix = [[0, 0, 3, 3, 0, 0], [0, 3, 2, 2, 3, 0], [3, 2, 2, 2, 2, 3], [3, 2, 2, 2, 2, 3], [0, 3, 2, 2, 3, 0], [0, 0, 3, 3, 0, 0]];
			this.BRMatrix = [[0, 0, 0, 3, 3, 3, 0, 0, 0], [0, 0, 3, 2, 2, 2, 3, 0, 0], [0, 3, 2, 2, 2, 2, 2, 3, 0], [3, 2, 2, 2, 2, 2, 2, 2, 3], [3, 2, 2, 2, 4, 2, 2, 2, 3], [3, 2, 2, 2, 2, 2, 2, 2, 3], [0, 3, 2, 2, 2, 2, 2, 3, 0], [0, 0, 3, 2, 2, 2, 3, 0, 0], [0, 0, 0, 3, 3, 3, 0, 0, 0]];
			this.riverEdge = [13, 13, 17, 15, 5, 2, 19, 17, 9, 11, 2, 13, 7, 9, 5, 2];
			this.treeTable = [0, 0, 0, 34, 0, 0, 36, 35, 0, 32, 0, 33, 30, 31, 29, 37];
		}
		construct(w, h, debug = false) {
			...
			if (Micro.TERRAIN_CREATE_ISLAND === 1) this.makeNakedIsland();else this.clearMap();
		...
		cleanBorder() {
			...
			for (x = 0; x < map.width; x++) {
				l = x + 1;
				if (l > map.width - 1) l = map.width - 1;
				r = x - 1;
				if (r < 0) r = 0;
				s = 1;
			...
			for (y = 0; y < map.height; y++) {
				l = y + 1;
				if (l > map.height - 1) l = map.height - 1;
				r = y - 1;
				if (r < 0) r = 0;
				s = 1;
		...
		makeNakedIsland() {
			...
			map.pp.forEach(v => {
				x = v[0];
				y = v[1];
				if (x < 5 || x >= map.width - 5 || y < 5 || y >= map.height - 5) map.setTile(x, y, Tile.RIVER, 0);else map.setTile(x, y, Tile.DIRT, 0);
			});
		...
		makeSingleLake(pos) {
			let numPlops = math.getRandom(12) + 2,
				plopPos;
			while (numPlops > 0) {
				plopPos = new Position(pos, math.getRandom(12) - 6, math.getRandom(12) - 6);
				if (math.getRandom(4)) this.plopSRiver(plopPos);else this.plopBRiver(plopPos);
				numPlops--;
			}
		}
		treeSplash(x, y) {
	...
	class WorldEffects {
		...
		setTile(x, y, value, flags) {
			...
			if (flags === undefined && !value.isTile) value = new Tiles(value);else if (flags !== undefined) value = new Tiles(value, flags);
			let key = this.toKey(x, y);
		...
		putBuilding(leftX, topY) {
			...
			for (let dy = 0; dy < this.size; dy++) {
				...
				for (let dx = 0; dx < this.size; dx++) {
					...
					if (dx === 1) {
						if (dy === 1) tileFlags |= Tile.ZONEBIT;else if (dy === 2 && this.animated) tileFlags |= Tile.ANIMBIT;
					}
	...
	class RoadTool extends BaseToolConnector {
		constructor(map) {
			super();
			this.doTool = function (x, y, blockMaps) {
	...
	class QueryTool extends BaseTool {
		...
		classifyLandValue(x, y, blockMaps) {
			...
			var i = 0;
			if (landValue >= 150) i = 3;else if (landValue >= 80) i = 2;else if (landValue >= 30) i = 1;
		...
		classifyZone(x, y) {
			var baseTiles = [Tile.DIRT, Tile.RIVER, Tile.TREEBASE, Tile.RUBBLE, Tile.FLOOD, Tile.RADTILE, Tile.FIRE, Tile.ROADBASE, Tile.POWERBASE, Tile.RAILBASE, Tile.RESBASE, Tile.COMBASE, Tile.INDBASE, Tile.PORTBASE, Tile.AIRPORTBASE, Tile.COALBASE, Tile.FIRESTBASE, Tile.POLICESTBASE, Tile.STADIUMBASE, Tile.NUCLEARBASE, Tile.HBRDG0, Tile.RADAR0, Tile.FOUNTAIN, Tile.INDBASE2, Tile.FOOTBALLGAME1, Tile.VBRDG0, 952];
	...
	class CityGame {
		...
		static post(e, buffer) {
			if (isWorker) postMessage(e, buffer);else returnMessage({
				data: e
			});
		}
	}
	...
	class MainGame {
			...
			for (var i = 0, l = messages.length; i < l; i++) {
				...
				switch (m.message) {
					...
					case Messages.QUERY_WINDOW_NEEDED:
						CityGame.post({
							tell: "QUERY",
							queryTxt: this.currentTool.getInfo()
						});
						break;
		...
		tool(name) {
			...
			if (name !== "none") this.currentTool = this.gameTools[name];else this.currentTool = null;
		}
		destroy(x, y) {
		...
		mapClick(x, y, single) {
			if (this.currentTool !== null) {
				...
				switch (this.currentTool.result) {
					case this.currentTool.TOOLRESULT_NEEDS_BULLDOZE:
						TXT.toolMessages.needsDoze;
						break;
					case this.currentTool.TOOLRESULT_NO_MONEY:
						TXT.toolMessages.noMoney;
						break;
					default:
						//if( id >= 11	&& id != 15 ) this.needMapUpdate = true;
						if (!single) CityGame.post({
							tell: "BUILD",
							x: x,
							y: y
						});
...
//# sourceMappingURL=data:application/json;charset=utf-8;base64,eyJ2ZXJzaW9uIjozLCJmaWxlIjoiY2l0eWdhbWUuanMiLCJzb3VyY2VzIjpbXSwic291cmNlc0NvbnRlbnQiOltdLCJuYW1lcyI6W10sIm1hcHBpbmdzIjoiIn0=
